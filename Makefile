all:
	...
	tools/bin2hex -s 3000 -n chars   -f assets/dustyChars.raw >> build/combined.s
	cd build ;/usr/local/bin/mac2c64 -r combined.s
	tools/linker build/combined.rwa build/combined.rwb build/combined.rwc > build/combined.prg
